...
if($app->getDirSetup()[0] === "site" && (preg_match("/($content)/", $app->getDirSetup()[1]))) {
    $docs = new ContentController($app->getContents('main'), $ignore_files);
    $docs->incContents();
}

if($app->getDirSetup()[0] === 'site') {
    $docs = new ContentController($app->getContents($app->getDirSetup()[1]), $ignore_files);
} else {
    $docs = new ContentController($app->getContents(''), $ignore_files);
}
$docs->incContents();
